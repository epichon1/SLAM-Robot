...
infrastructure. The `vertex_media/` directory beside this one supplies enough to run the
planning stack on a laptop:
...
cd ~/robotws
colcon build --base-paths vertex_media/pacman_msgs vertex_planning/packages \
                          vertex_planning/shared169 --symlink-install
...
```bash
ros2 launch vertex_media/demo.launch.py
```
...
east along the top, and back down the far side — in 76 seconds, replanning at each
...
Per-leg times for the recorded run, in seconds: 5.5, 10.5, 12.5, 5.0, 4.0, 11.0, 11.0,
3.5, 12.5. Planning itself is a small part of that — a 4 m leg plans in well under a
...
linear scan, so the work per step grows with the tree. Short legs are cheap: all nine
legs of the tour plan in under a second across repeated trials. A goal on the far
side of the maze regularly exhausts the 100 000-step cap instead — and because the
search runs inside the subscriber callback, the node is deaf for the whole time. Timed
offline against this map, two of the legs that were tried and rejected took up to four
minutes to fail. Keeping goals about 4 m apart is what makes the demo reliable.
